...
static function _get2desiredups(oRecord, oTabpage4, lOk2Get) // BACKS INTO A DESIRED OUT THE DOOR AMOUNT

   LOCAL GetList[0], GetOptions
   LOCAL nDelta := 0, nAddAmt := 1

   DEFAULT lOk2Get := .F.

   IF lOk2Get
     // GET THE STORED DESIRED OUT THE DOOR AMOUNT
     @   5.00,  0.00 DCSAY ""Enter the Out The Door Cash Amount To Be Paid By This Customer"
     @   6.00,  0.00 DCSAY "Note: Any Cash Deficiency Will Be Added To The Capitalized Cost"
     @   8.00,  0.00 DCSAY "                                              Out The Door Cash" GET oRecord:downpay PICTURE "99999.99"

     DCGETOPTIONS
       TITLE "Desired Out the Door"
       SAYWIDTH 0;
       AUTORESIZE;
       SAYFONT '10.Courier New';
       GETFONT '10.Courier New'

     DCREAD GUI MODAL ENTEREXIT ADDBUTTONS FIT OPTIONS GETOPTIONS OWNER oTabpage4
  ENDIF

  IF oRecord:lterm < 1               /// don't waste time if no term or residual percent
     return oRecord
  ENDIF
  ...
  ENDIF
   ...
   /// IF CURRENT INCEPTIONS ARE LESS THAT THE DESIRED AMOUNT LOOP THE FOLLOWING BY ADJUSTING CASHDOWN BY $1 UNTIL THEY ARE = TO PICKUPPAY
...
static function _makeLeasePayUps(oRecord)   /// NOTE THE oAcedeal:capccr USAGE IS FOR THE TAX ON CAPCOST REDUCTION - NOT THE FIRST PAYMENT
  ...
  IF Upper(oRecord:state) <> "NY"
   ...
   ENDIF
// THIS IS (just) A TEST LINE That we are adding to
...
static function _getToLeasePayUps(oRecord,oTabpage4)
   ...
   LOCAL GetList[0], GetOptions
   LOCAL nAmtReduce := 0, nNewPay := 0, nNewCapCost := 0, nApptosale := 0, nAppToDown := 0
   LOCAL oAcedealNew, oAdjMF, oToolbar5, oToolbar6, oAdjPrice, oToolbar, oToolbar2, oAdjdown, oToolbar3, oToolbar4
   LOCAL lStatus, lOk2Get := .F.
   ...
   @   1.00,  1.00 DCSAY "Monthly Payment" GET oRecord:lpayment2  PICTURE '$999999.99' EDITPROTECT {|| .T. }     // PARENT oAdjPrice
   @   2.00,  1.00 DCSAY "Current CapCost" GET oRecord:capcost    PICTURE '$999999.99' EDITPROTECT {|| .T. }     // PARENT oAdjPrice
   //@   2.00, 50.00 DCSAY "Current Vehicle Gross" GET oAceDeal:vehgross  PICTURE '$999999.99' EDITPROTECT {|| .T. } ;
   //                GETCOLOR {|| IIF( oAceDeal:vehgross > 0,{1,BD_KEYLIME} ,{1,BD_SALMON} )}

   @   3.00,  0.00 DCGROUP oAdjPrice CAPTION 'Adjust Price By Button'  SIZE 120.00,6.50

   @   3.00, 85.00 DCSAY "    Selling Price" GET oRecord:quoted  PICTURE '$999999.99' EDITPROTECT {|| .T. }  PARENT oAdjPrice

   @   1.00,  1.00 DCTOOLBAR oToolbar SIZE 100,2 BUTTONSIZE 10.00,2.00  PARENT oAdjPrice

   DCADDBUTTON CAPTION "Add .01" ;
     PARENT oToolbar ;
     ACTION {|| oRecord:quoted += .01, _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

   DCADDBUTTON CAPTION "Add .10" ;
     PARENT oToolbar ;
     ACTION {|| oRecord:quoted += .10 , _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

   DCADDBUTTON CAPTION "Add .50" ;
     PARENT oToolbar ;
     ACTION {|| oRecord:quoted += .50, _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

   DCADDBUTTON CAPTION "Add 1.00" ;
     PARENT oToolbar ;
     ACTION {|| oRecord:quoted += 1.00, _get2DesiredUps(oRecord,oTabPage4,lOk2Get), DC_GetRefresh(GetList)}

   DCADDBUTTON CAPTION "Add 5.00" ;
     PARENT oToolbar ;
     ACTION {|| oRecord:quoted += 5.00, _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

   DCADDBUTTON CAPTION "Add 10.00" ;
     PARENT oToolbar ;
     ACTION {|| oRecord:quoted += 10.00, _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

   DCADDBUTTON CAPTION "Add 50.00" ;
     PARENT oToolbar ;
     ACTION {|| oRecord:quoted += 50.00, _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

   DCADDBUTTON CAPTION "Add 100" ;
     PARENT oToolbar ;
     ACTION {|| oRecord:quoted += 100, _get2DesiredUps(oRecord, oTabPage4, lOK2Get), DC_GetRefresh(GetList)}

   @   4.00,  1.00 DCTOOLBAR oToolbar2 SIZE 100,2 BUTTONSIZE 10.00,2.00  PARENT oAdjPrice
   ...
   @  10.00,  0.00 DCGROUP oAdjDown CAPTION "Adjust Out The Door Cash By Button"  SIZE 120.00,7.00
   @   1.00,  1.00 DCTOOLBAR oToolbar3 SIZE 100.00, 2.00 BUTTONSIZE 10.00,2.00 PARENT oAdjDown
   @   3.00, 85.00 DCSAY "Out The Door Cash" GET oRecord:downpay PICTURE '$999999.99' EDITPROTECT {|| .T. } PARENT oAdjDown

   DCADDBUTTON CAPTION "Add .01" ;
     PARENT oToolbar3 ;
     ACTION {|| oRecord:downpay += .01,_get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

   DCADDBUTTON CAPTION "Add .10" ;
     PARENT oToolbar3 ;
     ACTION {|| oRecord:downpay += .10, _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

   DCADDBUTTON CAPTION "Add .50" ;
     PARENT oToolbar3 ;
     ACTION {|| oRecord:downpay += .50, _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

   DCADDBUTTON CAPTION "Add 1.00" ;
     PARENT oToolbar3 ;
     ACTION {|| oRecord:downpay += 1.00, _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

   DCADDBUTTON CAPTION "Add 5.00" ;
     PARENT oToolbar3 ;
     ACTION {|| oRecord:downpay += 5.00, _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

    DCADDBUTTON CAPTION "Add 10.00" ;
      PARENT oToolbar3 ;
      ACTION {|| oRecord:downpay += 10.00, _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

    DCADDBUTTON CAPTION "Add 50.00" ;
      PARENT oToolbar3 ;
      ACTION {|| oRecord:downpay += 50.00, _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

    DCADDBUTTON CAPTION "Add 100" ;
      PARENT oToolbar3 ;
      ACTION {|| oRecord:downpay += 100, _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

   @   4.00,  1.00 DCTOOLBAR oToolbar4 SIZE 100.00,2.00 BUTTONSIZE 10.00,2.00 PARENT oAdjDown
   ...
   @  17.00,  0.00 DCGROUP oAdjMF CAPTION "Adjust Interest Rate / Money Factor By Button"  SIZE 120.00,7.50

   @   1.00,  1.00 DCTOOLBAR oToolbar5 SIZE 80.00,2.00 BUTTONSIZE 10.00,2.00 PARENT oAdjMF

   @   3.00, 85.00 DCSAY "    Interest Rate" GET oRecord:leaserate1 PICTURE '999.99'   EDITPROTECT {|| .T.} PARENT oAdjMF
   @   6.00, 85.00 DCSAY "     Money Factor" GET oRecord:moneyfact1 PICTURE '9.999999' EDITPROTECT {|| .T.} PARENT oAdjMF


   DCADDBUTTON CAPTION "Add .01" ;
     PARENT oToolbar5 ;
     ACTION {|| oRecord:leaserate1 += .01, MakeMoneyFact(@oRecord), _get2DesiredUps(oRecord, oTabPage4, lOK2Get), DC_GetRefresh(GetList)}

   DCADDBUTTON CAPTION "Add .05" ;
     PARENT oToolbar5 ;
     ACTION {|| oRecord:leaserate1 += .05, MakeMoneyFact(@oRecord), _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

   DCADDBUTTON CAPTION "Add .10" ;
     PARENT oToolbar5 ;
     ACTION {|| oRecord:leaserate1 += .10, MakeMoneyFact(@oRecord), _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

   DCADDBUTTON CAPTION "Add .25" ;
     PARENT oToolbar5 ;
     ACTION {|| oRecord:leaserate1 += .25, MakeMoneyFact(@oRecord), _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

   DCADDBUTTON CAPTION "Add .50" ;
     PARENT oToolbar5 ;
     ACTION {|| oRecord:leaserate1 += .50, MakeMoneyFact(@oRecord), _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

   DCADDBUTTON CAPTION "Add .75" ;
     PARENT oToolbar5 ;
     ACTION {|| oRecord:leaserate1 += .75, MakeMoneyFact(@oRecord), _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

   DCADDBUTTON CAPTION "Add 1.00" ;
     PARENT oToolbar5 ;
     ACTION {|| oRecord:leaserate1 += 1.00, MakeMoneyFact(@oRecord), _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

   DCADDBUTTON CAPTION "Add 1.25" ;
     PARENT oToolbar5 ;
     ACTION {|| oRecord:leaserate1 += 1.25, MakeMoneyFact(@oRecord), _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}

   @   4.00,  1.00 DCTOOLBAR oToolbar6 SIZE 100.00,2.00 BUTTONSIZE 10.00,2.00 PARENT oAdjMF
   ...
   DCADDBUTTON CAPTION "Deduct 1.00" ;
     PARENT oToolbar6 ;
     ACTION {|| oRecord:leaserate1 -= 1.0, MakeMoneyFact(@oRecord), _get2DesiredUps(oRecord, oTabPage4, lOk2Get), DC_GetRefresh(GetList)}
   ...
   @  30.00,  1.00 DCSAY "                          Enter Desired Payment" GET nNewPay PICTURE '$99999.99';
                   VALID {|| _makeNewLeaseUps(nNewPay, @nNewCapCost, @nAmtReduce, oAceDealNew, oRecord), DC_GetRefresh(GetList), .T.} NOTABSTOP
   @  31.00,  1.00 DCSAY "Amount of CAPCOST Change Needed To Get Payment." GET nAmtreduce PICTURE '$99999.99' ;//EDITPROTECT {|| .T.};
                   GETCOLOR {|| IIF( nAmtReduce > 0, {1,BD_KEYLIME} ,{1,BD_SALMON} )  }

   DCGETOPTIONS ;
     TITLE "Get To Payment";
     SAYWIDTH 0 SAYFONT '10.Courier New';
     GETFONT '10.Courier New';
     AUTORESIZE

   DCREAD GUI TO lStatus FIT ADDBUTTONS OPTIONS GETOPTIONS OWNER oTabPage4
   IF !lStatus
     return nil
   ENDIF
...
FUNCTION PurgeAppRsl()
   ...
   LOCAL GetList[0], GetOptions
   LOCAL dStartDate := Date()-90,
   LOCAL oRecord, oDialog
   LOCAL lStatus
   ...
   @  10.00,  0.00 DCSAY "This Program Will Copy Appraisal Files To History That Fall Before The"
   @  11.00,  0.00 DCSAY "Below Entered Date"

   @  12.00,  0.00 DCSAY "Copy Appraisals To History That Are Dated Before" GET dStartdate

   DCGETOPTIONS;
     TITLE "Archive Appraisal Records";
     SAYWIDTH 0;
     SAYFONT '10.Courier New';
     GETFONT '10.Courier New';
     AUTORESIZE

   DCREAD GUI TO lStatus BUTTONS 2 ENTEREXIT OPTIONS GETOPTIONS FIT
